...
int main(int argc, char **argv) {
    ...
    int num_m = num_occurrences(str1, 'm');
    assert(num_m == 2);

    int num_n = num_occurrences(str1, 'n');
    assert(num_n == 0);
	...
	strcpy(dna_seq_3.sequence, "AAACCCTTTGGGG");
	...
	strcpy(dna_seq_4.sequence, "ATTTGGGG");
	compute_nucleotide_occurrences(&dna_seq_4);
	assert(dna_seq_4.A_count == 1);
	assert(dna_seq_4.C_count == 0);
	assert(dna_seq_4.G_count == 4);
	assert(dna_seq_4.T_count == 3);
